...
//MY(JULI) VERSION OF KEVIN'S CODE (can only read one april tag) NOT DONE!!!
public Pose2d getEstimatedGlobalPose() {
    PhotonPipelineResult results = camera.getLatestResult();

    System.out.println(camera.isConnected() + " " + camera.getName() + " " + camera.getPipelineIndex());


    if (results.hasTargets()) {
        Transform3d transform3d = results.getBestTarget().bestCameraToTarget;
...
//KEVIN'S CODE (can read multiple april tags???)
    // public Pose2d getEstimatedGlobalPose() {
